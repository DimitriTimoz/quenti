...
import { Alert, AlertIcon, Box, Center, Spinner, Text, VStack } from "@chakra-ui/react";
    ...
    const autoLogin = async () => {
      try {
        ...
        if (result?.ok) {
          ...
          setError("Authentification échouée. Vérifiez que les headers x-insa-auth-email et x-insa-auth-uid sont correctement définis.");
          setIsLoading(false);
          if (onComplete) onComplete();
        }
      } catch (err) {
        console.error("Auto-login failed:", err);
        setError("Une erreur est survenue lors de la tentative d'authentification automatique.");
        setIsLoading(false);
        if (onComplete) onComplete();
      }
    };
